...
from lib.hdf5_dataset import TempInputDataset
...
def train_app(cfg):
    ...
    #model = torch.load(Path.home() / 'crsp/ai4ts/afeeney/thermal_models/subcooled/FNO_temp_input_dataset.pt').cuda().float()
    model = torch.load(Path.home() / 'crsp/ai4ts/afeeney/thermal_models/wall_super_heat/FNO_temp_input_dataset.pt').cuda().float()
    val_dataset = TempInputDataset(cfg.dataset.val_paths[0], time_window=cfg.train.time_window)
